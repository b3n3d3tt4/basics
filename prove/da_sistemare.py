...
# Funzione generica per il fit
def fit(func, x, y, sx=None, sy=None, xlabel="X-axis", ylabel="Y-axis", initial_guess=None, title="Fit"):
    ...
    # Fitting con curve_fit
    if fit_with_weights:
        params, cov_matrix = curve_fit(func, x, y, p0=initial_guess, sigma=sigma_weights, absolute_sigma=True)
    else:
        params, cov_matrix = curve_fit(func, x, y, p0=initial_guess)

    uncertainties = np.sqrt(np.diag(cov_matrix))

    # Calcolo dei residui
    fit_values = func(x, *params)
    ...
    plt.plot(x, fit_values, color='red', label=title, lw=2)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.title(title)
    plt.grid(alpha=0.5)
    ...
    return params, residui, chi_squared, chi_squared_reduced

def regression(x, y, sx=None, sy=None, xlabel="X-axis", ylabel="Y-axis"):
    return fit(linear, x, y, sx, sy, xlabel, ylabel, [1, np.mean(y)], "Linear Fit")

def exponential(x, y, sx=None, sy=None, xlabel="X-axis", ylabel="Y-axis"):
    return fit(exp, x, y, sx, sy, xlabel, ylabel, [1, -1, np.mean(y)], "Exponential Fit")

def parabolic(x, y, sx=None, sy=None, xlabel="X-axis", ylabel="Y-axis"):
    return fit(parabola, x, y, sx, sy, xlabel, ylabel, [1, 1, np.mean(y)], "Parabolic Fit")

def wignerfit(x, y, sx=None, sy=None, xlabel="X-axis", ylabel="Y-axis"):
    return fit(wigner, x, y, sx, sy, xlabel, ylabel, [1, 1, np.mean(x)], "Breit-Wigner Fit")

def lorentzian(x, y, sx=None, sy=None, xlabel="X-axis", ylabel="Y-axis"):
    return fit(lore, x, y, sx, sy, xlabel, ylabel, [1, 1, np.mean(x)], "Lorentzian Fit")

def voigtfit(x, y, sx=None, sy=None, xlabel="X-axis", ylabel="Y-axis"):
    return fit(voigt, x, y, sx, sy, xlabel, ylabel, [max(y), np.mean(x), np.std(x), 1], "Voigt Fit")